#ifndef _MEMORY_ARRAY_LIST_HPP_INCLUDED_DSOIJU4E8UFA98Y498YUASFIOHJASFKJHVUJHA98Y4U8FSDOIUAOFIUR89FF
#define _MEMORY_ARRAY_LIST_HPP_INCLUDED_DSOIJU4E8UFA98Y498YUASFIOHJASFKJHVUJHA98Y4U8FSDOIUAOFIUR89FF
...
namespace feng
{
    namespace memory_array_list_private
    {
    ...
    struct memory_array_list
    {
        ...
        {
            n = memory_array_list_private::left_most_position()(n);
            n = n > 6 ? n : 6;
            size_t N = 1 << n;

            mutex_available_tree_.lock();
            // 1) 2) 2.1)
            if ( !(available_tree_[n].empty()) )
            {
                auto mem = available_tree_[n].front();
                // 2.2)
                lock_guard_type lo { mutex_occupied_tree_ };
                occupied_tree_[n].push_front( mem );
                // 2.3)
                available_tree_[n].pop_front();
                return mem;
            }
...
#endif//_MEMORY_ARRAY_LIST_HPP_INCLUDED_DSOIJU4E8UFA98Y498YUASFIOHJASFKJHVUJHA98Y4U8FSDOIUAOFIUR89FF
